...
query = image_type
url = f"http://www.bing.com/images/search?q={query}&qft=+filterui:color2-bw+filterui:imagesize-large&FORM=R5IR3"
soup = get_soup(url)
...
for img in images:
   ...
   with open(f"images/{image_type}_{str(cntr)}", 'wb') as f:
      f.write(raw_img)
